...
/**
 * Generate gaussian distributed, fully random Data
 * @author Sven Wiemann
 *
 */
public class DataGenRandomGauss implements DataGenInterface{
	private static final Logger logger = Logger.getLogger(DataGenRandomGauss.class.getName());
	private long seed;
	 ...
	 * @param seed
	 */
	public DataGenRandomGauss(long seed) {
		this.seed = seed;
	}
	 ...
	 */
	public DataGenRandomGauss() {
		this(System.currentTimeMillis());
	}
	
	/**
	 * Generate custom seeded Data in an Interval.
	 * @param seed
	 * @param lowerBounds
	 * @param higherBounds
	 */
	public DataGenRandomGauss(long seed, float lowerBounds, float higherBounds){
		this(seed);
	...
	/**
	 * Generate Data in an Interval
	 * @param lowerBounds
	 * @param higherBounds
	 */
	public DataGenRandomGauss(float lowerBounds, float higherBounds){
		this(System.currentTimeMillis());
		this.lowerBounds = lowerBounds;
		this.higherBounds = higherBounds;		
	}
	...
	public TupleList genData(int dimensions, int tupleCount) {
		logger.info("Generating gaussian random Data with " + tupleCount + " Tuples in " + dimensions + " dimensions");
		...
		for (int i = 0; i <= tupleCount; i++){
			...
			for(int j = 0; j < dimensions; j++){
				float value = lowerBounds + ((float) gen.nextGaussian()) * ((higherBounds - lowerBounds) + 1);
