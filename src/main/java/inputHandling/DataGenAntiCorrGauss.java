...
/**
 * Generate Gaussian Distributed anti correlated Data
 * One Component of every Tuple must be smaller than the center of the Distribution
 * @author Sven Wiemann
 *
 */
public class DataGenAntiCorrGauss implements DataGenInterface{
	private static final Logger logger = Logger.getLogger(DataGenAntiCorrGauss.class.getName());
	private long seed;
	 ...
	 * @param seed
	 */
	public DataGenAntiCorrGauss(long seed) {
		this.seed = seed;
	}
	 ...
	 */
	public DataGenAntiCorrGauss(long seed, float std, float center){
		this(seed);
		this.std = std;
		this.center = center;		
	}
	 ...
	 * @param center center of the distribution
	 */
	public DataGenAntiCorrGauss(float std, float center){
		this(System.currentTimeMillis());
		this.std = std;
		this.center = center;		
	}
	...
	public TupleList genData(int dimensions, int tupleCount) {
		logger.info("Generating gaussian anti-correlated Data with " + tupleCount + " Tuples in " + dimensions + " dimensions");
